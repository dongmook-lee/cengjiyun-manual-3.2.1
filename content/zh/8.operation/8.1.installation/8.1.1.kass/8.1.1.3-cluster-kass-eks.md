      ...
      * 권한 추가(Add additional permissions)를 선택합니다.  
        ...
        * **CloudFormation** - 목록 에서 액세스 레벨 / 리소스 에서 필요한 권한을 선택합니다.  
          ...
          AWS 리소스를 개별적으로 생성하고 구성 할 필요가 없으며 무엇에 의존하는지 파악할 수 있습니다. [Learn more](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/)  
            ...
            ![eks-in-seoul]({{< param imageRootDir >}}/assets/ZH/{{< param version >}}/EKS/eks-create-policies-CloudFormation-1.png)
            ![eks-in-seoul]({{< param imageRootDir >}}/assets/ZH/{{< param version >}}/EKS/eks-create-policies-CloudFormation-2.png)
            ![eks-in-seoul]({{< param imageRootDir >}}/assets/ZH/{{< param version >}}/EKS/eks-create-policies-CloudFormation-3.png)
        ...
        * **EKS** - 목록 에서 액세스 레벨 / 리소스 에서 필요한 권한을 선택합니다.  
          Kubernetes 용 Amazon Elastic Container Service (Amazon EKS)는 자체 Kubernetes control plane을 세우거나 유지할 필요없이 AWS에서 Kubernetes를 쉽게 실행할 수 있도록 해주는 관리 형 서비스입니다.  
            ...
            ![eks-in-seoul]({{< param imageRootDir >}}/assets/ZH/{{< param version >}}/EKS/eks-create-policies-EKS-1.png)
        ...
        * **IAM** - 목록 에서 액세스 레벨 / 리소스 에서 필요한 권한을 선택합니다.  
          ...
          IAM을 사용하여 자원을 사용하도록 인증 (로그인)되고 권한이 부여 된 (사용 권한이있는) 사용자를 제어합니다.  
            ...
            ![eks-in-seoul]({{< param imageRootDir >}}/assets/ZH/{{< param version >}}/EKS/eks-create-policies-IAM-1.png)
        ...
        * **SNS** - 목록 에서 액세스 레벨 / 리소스 에서 필요한 권한을 선택합니다.  
          ...
          게시자는 논리 액세스 포인트 및 통신 채널 인 주제에 메시지를 생성하고 전송하여 구독자와 비동기 적으로 통신합니다.  
            ...
            ![eks-in-seoul]({{< param imageRootDir >}}/assets/ZH/{{< param version >}}/EKS/eks-create-policies-SNS-1.png)
...
* [IAM(Identity and Access Management)](https://console.aws.amazon.com/iam) 에서 사용자(Users)를 선택합니다.  
    ...
    * 생성한 또는 편집한 정책을 사용자에게 추가 합니다.
    ...
    ![eks-in-seoul]({{< param imageRootDir >}}/assets/ZH/{{< param version >}}/EKS/eks-create-user-policies-1.png)
...
이 사용 설명서에서는 직접 생성을 선택 합니다.  
  ...
  * VPC 만들기(Launch VPC Wizard)를 선택합니다.
    ...
    * 1단계: VPC 구성 선택(Step 1: Select a VPC Configuration) :  
    ...
    ![eks-in-seoul]({{< param imageRootDir >}}/assets/ZH/{{< param version >}}/EKS/eks-create-vpc-1.png)
  ...
  * Your VPCs 화면 목록에서 생성된 VPC를 확일 할 수 있습니다.
  ...
  ![eks-in-seoul]({{< param imageRootDir >}}/assets/ZH/{{< param version >}}/EKS/eks-create-vpc-2.png)
  ![eks-in-seoul]({{< param imageRootDir >}}/assets/ZH/{{< param version >}}/EKS/eks-create-vpc-3.png)  
...
클러스터를 만들 때 해당 클러스터를 위해 리소스를 호스팅할 모든 서브넷을 지정합니다(예: 작업자 노드와 로드 밸런서).
  ...
  * VPC Service 의 서브넷(Subnets) 화면에서 확인할 수 있습니다.
  ...
  ![eks-in-seoul]({{< param imageRootDir >}}/assets/ZH/{{< param version >}}/EKS/eks-create-subnet-1.png)
  ![eks-in-seoul]({{< param imageRootDir >}}/assets/ZH/{{< param version >}}/EKS/eks-create-subnet-2.png)
...
* AWS VPC service 화면에서 보안 그룹을 선택합니다.
    ...
    * Create를 선택합니다.
    ...
    ![eks-create-security-group-1]({{< param imageRootDir >}}/assets/ZH/{{< param version >}}/EKS/eks-create-security-group-1.png)
 -->
...
* AWS EKS 콘솔을 사용하여 클러스터를 생성 합니다.
    ...
    * **5.** Status(상태) 필드는 클러스터 프로비저닝 프로세스가 완료될 때까지 CREATING(생성 중)으로 표시됩니다.  
        ...
        ![eks-create-cluster-1]({{< param imageRootDir >}}/assets/ZH/{{< param version >}}/EKS/eks-create-cluster-1.png)
...
* 목록에서 생성한 인스턴스 이름을 선택한 다음 작업을 선택합니다.
    ...
    * 열린 메뉴에서 네트워킹과 보안 그룹 변경을 차례대로 선택합니다.
    ...
    ![efs-create-security-group-1]({{< param imageRootDir >}}/assets/ZH/{{< param version >}}/EKS/efs-create-security-group-1.png)

    * 기본 VPC 보안 그룹이라는 설명이 있는 보안 그룹 옆 확인란을 선택합니다.

    ![efs-create-security-group-2]({{< param imageRootDir >}}/assets/ZH/{{< param version >}}/EKS/efs-create-security-group-2-1.png)
    ...
    * 설명 탭에서 보안 그룹 – 옆에 두 항목이 나열되어 있는지 확인합니다. 하나는 기본 VPC 보안 그룹용이고 다른 하나는 인스턴스 시작 시 생성했던 보안 그룹용입니다.
    ...
    ![efs-create-security-group-3]({{< param imageRootDir >}}/assets/ZH/{{< param version >}}/EKS/efs-create-security-group-3.png)
